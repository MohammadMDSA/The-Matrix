...
void Game::Initialize(Windows::UI::Core::CoreWindow^ window, int width, int height, int rotation)
{
    ...
    m_outputRotation = (DXGI_MODE_ROTATION)rotation;
...
void Game::OnWindowSizeChanged(int width, int height, int rotation)
{
    m_outputWidth = std::max(width, 1);
    m_outputHeight = std::max(height, 1);
    m_outputRotation = (DXGI_MODE_ROTATION)rotation;
...
// Properties
int Game::GetDefaultWidth()
{
	return 800;
}

int Game::GetDefaultHeight()
{
